...
export default function RegisterIngresoPage() {
    ...
    const [ubicacion, setUbicacion] = useState<'almacen' | 'cafetin'>('almacen');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (motivo === 'compra') {
                ...
                const currentStock = ubicacion === 'almacen' ? (selectedProduct?.stockAlmacen || 0) : (selectedProduct?.stockCafetin || 0);
                const newStock = currentStock + numCantidad;
            ...
            <PageContainer>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        {selectedProduct && (
                            <p className="mt-2 text-sm text-gray-500">
                                Stock actual ({ubicacion === 'almacen' ? 'Almacén' : 'Cafetín'}):{' '}
                                <span className="font-medium">
                                    {ubicacion === 'almacen' ? selectedProduct.stockAlmacen : selectedProduct.stockCafetin}
                                </span>{' '}
                                {selectedProduct.baseUnit}
                            </p>
                        )}
                    ...
                    <div>
                        ...
                        <select
                            value={ubicacion}
                            onChange={(e) => setUbicacion(e.target.value as 'almacen' | 'cafetin')}
                            className="block w-full h-10 px-3 border border-gray-300 rounded-md leading-5 bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                        >
